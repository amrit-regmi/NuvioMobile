...
/**
 * Single source of truth for how aggregated ratings render across the app (details screen +
 * home hero). Mirrors NuvioTV's unified rating row: fold the inline meta imdb value into the
 * aggregated (/MDBList) set, dedupe on imdb, then place a SINGLE rating inline and TWO OR MORE
 * on their own scrollable row.
 */

/**
 * Merges the standalone [imdbRatingString] (e.g. from a catalog `imdbRating` field or
 * `MetaDetails.imdbRating`) into [externalRatings], preferring an aggregated imdb value when one
 * is already present. Result participates in the ≥2-own-row / 1-inline placement rule.
 */
fun unifyAggregatedRatings(
    externalRatings: List<MetaExternalRating>,
    imdbRatingString: String?,
): List<MetaExternalRating> {
    val hasAggregatedImdb = externalRatings.any { it.source == PROVIDER_IMDB }
    val metaImdbValue = imdbRatingString
        ?.toDoubleOrNull()
        ?.takeIf { it > 0.0 }
    return if (!hasAggregatedImdb && metaImdbValue != null) {
        listOf(MetaExternalRating(source = PROVIDER_IMDB, value = metaImdbValue)) + externalRatings
    } else {
        externalRatings
    }
}
...
/**
 * Renders the given [ratings] in canonical provider order using the shared visuals. When [ownRow]
 * is true it fills width and horizontally scrolls (details/home ratings row); when false it wraps
 * its content for inline placement on a genre/year meta row.
 */
@Composable
fun AggregatedRatingsRow(
